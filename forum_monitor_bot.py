...
def get_all_folders(imap):
    """Получить список всех папок из IMAP (с поддержкой IMAP-литералов)."""
    ...
    if status == "OK":
        for item in folders:
            ...
            if isinstance(item, tuple):
                # Ответ с литералом: (b'(...) "/" {N}', b'имя_папки')
                head, literal = item
                name = literal.decode("utf-8", errors="ignore") if isinstance(literal, bytes) else str(literal)
            else:
                line = item.decode(errors="ignore") if isinstance(item, bytes) else str(item)
